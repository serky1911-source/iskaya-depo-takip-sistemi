...
# Render.com bize bir "DATABASE_URL" verecek. Onu kullanacağız.
def get_db_connection():
    # Eğer sistemde DATABASE_URL tanımlıysa (Bulut) onu kullan, yoksa yerel postgres'e bağlan
    database_url = os.environ.get('DATABASE_URL')
    
    if not database_url:
        # Yerelde test ederken hata vermemesi için (Opsiyonel)
...
# --- Tabloları Oluşturma (PostgreSQL Uyumlu) ---
def veritabani_kur():
    # Bağlantı kurulana kadar bekle (Bulut bazen geç uyanır)
    retries = 5
    while retries > 0:
        try:
            ...
            # Admin ve Müdür Ekleme (Conflict durumunu Postgres'e göre yönetiyoruz)
            ...
            if not cursor.fetchone():
                cursor.execute("INSERT INTO Kullanicilar (kullanici_adi, sifre, rol) VALUES ('mudur', 'mudur123', 'izleyici')")
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS StokCikis (
                    cikis_id SERIAL PRIMARY KEY,
                    urun_id INTEGER REFERENCES Urunler(urun_id),
                    bolum_id INTEGER REFERENCES Bolumler(bolum_id),
                    miktar REAL NOT NULL,
                    cikis_tarihi TIMESTAMP NOT NULL,
                    sevkiyat_no VARCHAR(100),
                    aciklama TEXT
                );
            """)
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS Zimmetler (
                    zimmet_id SERIAL PRIMARY KEY,
                    urun_id INTEGER REFERENCES Urunler(urun_id),
                    personel_id INTEGER REFERENCES Personel(personel_id),
                    miktar REAL NOT NULL,
                    verilis_tarihi TIMESTAMP NOT NULL
                );
            """)

            conn.commit()
            conn.close()
            print("Veritabanı tabloları başarıyla hazırlandı.")
            break
        except Exception as e:
            print(f"Veritabanı hazırlığı deneniyor... ({retries}) Hata: {e}")
            retries -= 1
            time.sleep(2)
...
# --- UYGULAMA BAŞLANGICI ---
# Bulutta uygulama başladığında veritabanını kur
...
@app.get("/")
def ana_sayfa():
    return {"mesaj": "Depo Takip Sistemi Bulutta Çalışıyor! ☁️"}

# --- ENDPOINTLER (PostgreSQL Uyumlu) ---
...
def bolumleri_getir():
    ...
    cur.execute("SELECT * FROM Bolumler")
...
def bolum_ekle(b: BolumModel):
    conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO Bolumler (bolum_adi) VALUES (%s)", (b.bolum_adi,))
        conn.commit()
...
def personel_ekle(p: PersonelModel):
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("INSERT INTO Personel (ad_soyad) VALUES (%s)", (p.ad_soyad,))
...
def urun_ekle(u: UrunModel):
    conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO Urunler (urun_adi, SKU, birim, tur, guvenlik_stogu) VALUES (%s,%s,%s,%s,%s)", 
                    (u.urun_adi, u.sku, u.birim, u.tur, u.guvenlik_stogu))
        conn.commit()
    except Exception: pass
    conn.close(); return {"msg": "ok"}
...
def stok_giris(i: IslemModel):
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT urun_id FROM Urunler WHERE SKU=%s", (i.sku,))
    urun = cur.fetchone()
    cur.execute("INSERT INTO StokGiris (urun_id, bolum_id, miktar, giris_tarihi, belge_no) VALUES (%s,%s,%s,%s,%s)", 
                (urun['urun_id'], i.bolum_id, i.miktar, datetime.now(), i.belge_no))
    conn.commit(); conn.close(); return {"msg": "ok"}
...
def stok_cikis(i: IslemModel):
    ...
    cur.execute("INSERT INTO StokCikis (urun_id, bolum_id, miktar, cikis_tarihi, sevkiyat_no, aciklama) VALUES (%s,%s,%s,%s,%s,%s)", 
                (urun['urun_id'], i.bolum_id, i.miktar, datetime.now(), i.belge_no, "Tüketim"))
    conn.commit(); conn.close(); return {"msg": "ok"}

@app.post("/stok/transfer")
def transfer(t: TransferModel):
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT urun_id FROM Urunler WHERE SKU=%s", (t.sku,))
    urun = cur.fetchone()
    now = datetime.now()
    cur.execute("INSERT INTO StokCikis (urun_id, bolum_id, miktar, cikis_tarihi, aciklama) VALUES (%s,%s,%s,%s,%s)", 
                (urun['urun_id'], t.cikis_bolum_id, t.miktar, now, f"Transfer->{t.giris_bolum_id}"))
    cur.execute("INSERT INTO StokGiris (urun_id, bolum_id, miktar, giris_tarihi, belge_no) VALUES (%s,%s,%s,%s,%s)", 
                (urun['urun_id'], t.giris_bolum_id, t.miktar, now, f"Transfer<-{t.cikis_bolum_id}"))
    conn.commit(); conn.close(); return {"msg": "ok"}

@app.post("/zimmet/ver")
def zimmet(z: ZimmetModel):
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT urun_id FROM Urunler WHERE SKU=%s", (z.sku,))
    urun = cur.fetchone()
    now = datetime.now()
    cur.execute("INSERT INTO StokCikis (urun_id, bolum_id, miktar, cikis_tarihi, aciklama) VALUES (%s,%s,%s,%s,%s)", 
                (urun['urun_id'], z.bolum_id, z.miktar, now, f"Zimmet->{z.personel_id}"))
    cur.execute("INSERT INTO Zimmetler (urun_id, personel_id, miktar, verilis_tarihi) VALUES (%s,%s,%s,%s)", 
                (urun['urun_id'], z.personel_id, z.miktar, now))
    conn.commit(); conn.close(); return {"msg": "ok"}
...
def genel_stok():
    conn = get_db_connection(); cur = conn.cursor()
    sorgu = """
    SELECT b.bolum_adi, u.urun_adi, u.SKU, u.tur, u.guvenlik_stogu,
        COALESCE(SUM(g.miktar), 0) - COALESCE((SELECT SUM(c.miktar) FROM StokCikis c WHERE c.urun_id = u.urun_id AND c.bolum_id = b.bolum_id), 0) as mevcut_stok
    FROM Bolumler b CROSS JOIN Urunler u LEFT JOIN StokGiris g ON u.urun_id = g.urun_id AND b.bolum_id = g.bolum_id
    GROUP BY b.bolum_id, u.urun_adi, u.SKU, u.tur, u.guvenlik_stogu, u.urun_id 
...
def zimmet_listesi():
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT z.verilis_tarihi, p.ad_soyad, u.urun_adi, u.SKU, z.miktar FROM Zimmetler z JOIN Personel p ON z.personel_id=p.personel_id JOIN Urunler u ON z.urun_id=u.urun_id")
    res = cur.fetchall(); conn.close(); return res
...
def hareket_dokumu(f: RaporFiltreModel):
    conn = get_db_connection(); cur = conn.cursor()
    sorgu = """
    SELECT 'GİRİŞ' as islem_tipi, g.giris_tarihi as tarih, b.bolum_adi, u.urun_adi, g.miktar, g.belge_no as aciklama
    FROM StokGiris g JOIN Bolumler b ON g.bolum_id = b.bolum_id JOIN Urunler u ON g.urun_id = u.urun_id WHERE g.giris_tarihi BETWEEN %s AND %s
    UNION ALL
    SELECT 'ÇIKIŞ' as islem_tipi, c.cikis_tarihi as tarih, b.bolum_adi, u.urun_adi, c.miktar, c.aciklama
    FROM StokCikis c JOIN Bolumler b ON c.bolum_id = b.bolum_id JOIN Urunler u ON c.urun_id = u.urun_id WHERE c.cikis_tarihi BETWEEN %s AND %s ORDER BY tarih DESC
    """
